...
export default function Dashboard() {
  ...
  const handleTileClick = (type: 'paint' | 'service' | 'plant', id: string) => {
    switch (type) {
      case 'paint':
        navigate('/paint-records', { state: { editId: id } });
        break;
      case 'service':
        navigate('/home-services', { state: { editId: id } });
        break;
      case 'plant':
        navigate('/plant-inventory', { state: { editId: id } });
        break;
    }
  };
                  ...
                  <div className="border-t border-gray-200 pt-3">
                    {combinedReminders.slice(0, 5).map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={`flex items-center justify-between py-2 px-3 rounded-lg mb-2 ${getThemeClass(itemTheme, 'background')} cursor-pointer hover:opacity-90 transition-opacity`}
                          onClick={() => {
                            if (item.type === 'reminder') {
                              navigate('/house-reminders', { state: { editId: item.id } });
                            } else {
                              navigate('/home-services', { state: { editId: item.id } });
                            }
                          }}
                        >
        ...
        <div className="lg:col-span-1">
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="p-5">
              ...
              {recentPaint && (
                <div
                  className={`overflow-hidden shadow rounded-lg mb-4 ${getThemeClass('paint', 'background')} cursor-pointer hover:opacity-90 transition-opacity`}
                  onClick={() => handleTileClick('paint', recentPaint.id)}
                >
                  <div className="p-4">
                    <div className="flex items-center">
                      <div className="flex-shrink-0">
                      ...
                      <div className="ml-3">
                        ...
                        <p className={`text-sm ${getThemeClass('paint', 'textLight')}`}>{recentPaint.color}</p>
              ...
              {recentService && (
                <div
                  className={`overflow-hidden shadow rounded-lg mb-4 ${getThemeClass('service', 'background')} cursor-pointer hover:opacity-90 transition-opacity`}
                  onClick={() => handleTileClick('service', recentService.id)}
                >
                  <div className="p-4">
                    <div className="flex items-center">
                      <div className="flex-shrink-0">
              ...
              {recentPlant && (
                <div
                  className={`overflow-hidden shadow rounded-lg ${getThemeClass('plant', 'background')} cursor-pointer hover:opacity-90 transition-opacity`}
                  onClick={() => handleTileClick('plant', recentPlant.id)}
                >
                  <div className="p-4">
                    <div className="flex items-center">
                      <div className="flex-shrink-0">
